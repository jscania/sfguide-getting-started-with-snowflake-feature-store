...
def uc01_load_data(
    order_data: DataFrame, lineitem_data: DataFrame, order_returns_data: DataFrame
) -> DataFrame:
    """
...
def uc01_pre_process(data: DataFrame) -> DataFrame:
    ...
    data = data.with_columns(
        ["INVOICE_YEAR", "ROW_PRICE", "RETURN_ROW_PRICE"],
        [
            F.year(data["ORDER_DATE"]),
            data["QUANTITY"] * data["PRICE"],
            data["OR_RETURN_QUANTITY"] * data["PRICE"],
        ],
    )
    ...
    groups = groups.withColumn(
        "RATIO", groups["RETURN_ROW_PRICE"] / groups["ROW_PRICE"]
    )
    ratio = groups.groupBy("O_CUSTOMER_SK").agg(
        F.avg(F.col("RATIO")).cast(T.FloatType()).alias("RETURN_RATIO"),
        F.max(F.col("LATEST_ORDER_DATE")).alias("LATEST_ORDER_DATE"),
    )

    # Calculate average annual shopping FREQUENCY
    frequency_groups = groups.groupBy("O_CUSTOMER_SK", "INVOICE_YEAR").agg(
        F.count(F.col("O_ORDER_ID")).cast(T.FloatType()).alias("FREQUENCY")
    )
    frequency = frequency_groups.groupBy("O_CUSTOMER_SK").agg(
        F.avg(F.col("FREQUENCY")).alias("FREQUENCY")
    )
